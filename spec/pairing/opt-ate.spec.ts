import { G1, G2, GT } from '../../src/pairing/bn';

describe('Optimal Ate Tests', () => {
    it('should get one', () => {
        const b = GT.pair(new G1(123n), new G2(456n));

        const a = new GT();
        a.neg(b);

        const one = new GT();

        one.add(a, b);
        expect(one.isOne()).toBeTruthy();
    });

    it('should pass the bilinearity test', () => {
        const v = '';
        const p1 = new G1(12345n);
        const p2 = new G2(67890n);

        const e1 = GT.pair(p1, p2);
        const e2 = GT.pair(new G1(1n), new G2(1n));
        e2.scalarMul(e2, 12345n);
        e2.scalarMul(e2, 67890n);

        expect(e1.marshal().toString('hex')).toBe(v);

        const minusE2 = new GT();
        minusE2.neg(e2);
        e1.add(e1, minusE2);

        expect(e1.isOne()).toBeTruthy();
    });

    it('should pair infinity points', () => {
        const p1 = new G1();
        p1.setInfinity();
        const p2 = new G2(123n);

        const e1 = GT.pair(p1, p2);

        expect(e1.isOne()).toBeTruthy();
    });
});
