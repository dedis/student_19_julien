import { G1, G2, GT } from '../../src/pairing/bn';

describe('Optimal Ate Tests', () => {
    it('should get one', () => {
        const b = GT.pair(new G1(BigInt(123)), new G2(BigInt(456)));

        const a = new GT();
        a.neg(b);

        const one = new GT();

        one.add(a, b);
        expect(one.isOne()).toBeTruthy();
    });

    it('should pass the bilinearity test', () => {
        const v = '';
        const p1 = new G1(BigInt(12345));
        const p2 = new G2(BigInt(67890));

        const e1 = GT.pair(p1, p2);
        const e2 = GT.pair(new G1(BigInt(1)), new G2(BigInt(1)));
        e2.scalarMul(e2, BigInt(12345));
        e2.scalarMul(e2, BigInt(67890));

        expect(e1.marshal().toString('hex')).toBe(v);

        const minusE2 = new GT();
        minusE2.neg(e2);
        e1.add(e1, minusE2);

        expect(e1.isOne()).toBeTruthy();
    });

    it('should pair infinity points', () => {
        const p1 = new G1();
        p1.setInfinity();
        const p2 = new G2(BigInt(123));

        const e1 = GT.pair(p1, p2);

        expect(e1.isOne()).toBeTruthy();
    });
});
